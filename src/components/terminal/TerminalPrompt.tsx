...
export default function TerminalPrompt({ onCommand, currentSection }: TerminalPromptProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-navy-800 border-t border-navy-600 z-50">
        ...
        <div className="flex items-center gap-2 text-sm font-mono">
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex items-center">
            <input
              ...
              className="flex-1 bg-transparent text-text-primary font-mono outline-none px-2"
              placeholder={currentSection ? `Try: ${COMMANDS.filter(c => c !== currentSection).join(', ')}` : 'Type a command...'}
              autoFocus
            />
